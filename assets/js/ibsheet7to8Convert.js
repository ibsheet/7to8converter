var convertSheet = {},notice="";

//원본 소스 앞,뒤  공백 트림
function legacyCodeTrim(){
    var legacyCode = document.querySelector("#lcode").value;
    document.querySelector("#lcode").value = 
    legacyCode.split("\r\n").join("").split(";").map(lines => lines.trim()).join(";\r\n");
}
...
function findSheetId(){
    ...
    if(legacyCode.indexOf("IBS_InitSheet(")>-1) {
        var start_pos = legacyCode.indexOf("IBS_InitSheet(")+14;
        var mid_pos = legacyCode.indexOf(",",start_pos);
        var end_pos = legacyCode.indexOf(")",mid_pos+1);
        document.querySelector("#sheetID").value = legacyCode.substring(start_pos,mid_pos).trim();
        document.querySelector("#initObj").value = legacyCode.substring(mid_pos+1,end_pos).trim();
    }else{
        if(legacyCode.indexOf("with(")>-1) {
            var start_pos = legacyCode.indexOf("with(")+5;
            var end_pos = legacyCode.indexOf(")",start_pos);
            document.querySelector("#sheetID").value = legacyCode.substring(start_pos,end_pos).trim();
        }
        document.querySelector("#initObj").value = "";
    }
    // 임의의 시트 객체
    window[document.querySelector("#initObj").value] = {};
...
function createIBSheetObject(){
    ...
    var sheetid = document.querySelector("#sheetID").value;
    //해당하는 이름으로 객체를 생성
    window[sheetid] = {};
...
//최종 변환!!
function convertIBSheet7ToIBSheet8(){
    var legacyCode = document.querySelector("#lcode").value;
    ...
    var initObj = document.getElementById("initObj").value;
    initObj = initObj!=""?initObj:"OPT";
    var elObj  = document.getElementById("elObj").value;
...
function zoomCode(code){
  var orgCode = document.querySelector(".convertMain div:first-child").style.display;
  var covCode = document.querySelector(".convertMain div:last-child").style.display;
  if(code == "original") {
    if(orgCode != "none" && covCode != "none") {
      document.querySelector(".convertMain div:last-child").style.display = "none"
    }else if(orgCode == "none" && covCode != "none") {
      document.querySelector(".convertMain div:first-child").style.display = "";
    }
  }else {
    if(orgCode != "none" && covCode != "none") {
      document.querySelector(".convertMain div:first-child").style.display = "none"
    }else if(orgCode != "none" && covCode == "none") {
      document.querySelector(".convertMain div:last-child").style.display = "";
    }
  }
}
var methodConvert = [
    {
        methodName : "DataInsert",
        chgName : "addRow",
        paramChg : function(param){
...
function convertFunction(){
    var lcode = document.querySelector("#lcode").value;
    var codeArr = lcode.match(/\w+|[^\w]/g);
    for(var idx = 0 ; idx < codeArr.length; i++){
        if(word==="DataInsert"){
            codeArr[idx] = "addRow"
        }
    }
